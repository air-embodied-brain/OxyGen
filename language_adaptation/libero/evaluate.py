...
from language_adaptation.libero import train
...
def _set_adapter(state: nnx.State, arrays: dict[str, np.ndarray]) -> None:
    for path, variable in state.filter(adapter.PARAM_FILTER).flat_state().items():
...
@dataclasses.dataclass(frozen=True)
class _EvaluationFunctions:
    prefill_root: Callable
    prefill_private: Callable
    suffix_step: Callable
    suffix_block: Callable
    request_active: Callable
    request_disabled: Callable
    actions: Callable


def _make_functions(model_def, *, suffix_len: int, seed_len: int):
    @jax.jit
    def prefill_root(state, observation):
        model = nnx.merge(model_def, state)
        return model.prefill_language_root(None, observation, train=False)

    @jax.jit
    def prefill_private(state, observation):
        model = nnx.merge(model_def, state)
    ...
    @jax.jit
    def suffix_step(state, kv_cache, prefix_mask, token, suffix_index):
        model = nnx.merge(model_def, state)
    ...
    @jax.jit
    def suffix_block(state, kv_cache, prefix_mask, tokens, mask):
        model = nnx.merge(model_def, state)
    ...
    return _EvaluationFunctions(
        prefill_root=prefill_root,
        prefill_private=prefill_private,
        suffix_step=suffix_step,
        suffix_block=suffix_block,
        request_active=make_request(adapter_active=True),
        request_disabled=make_request(adapter_active=False),
        actions=actions,
    )
...
def _generate(
    ...
    observation,
    tokenizer,
    functions: _EvaluationFunctions,
    *,
    suffix_len: int,
    suffix_seed: str,
) -> str:
    kv_cache, prefix_mask = functions.prefill_private(state, observation)
    ...
    seed_logits, kv_cache = functions.suffix_block(state, kv_cache, prefix_mask, seed_inputs, seed_mask)
    ...
    for generation_index in range(max_new_tokens):
        ...
        if generation_index + 1 == max_new_tokens:
            break
        logits, kv_cache = functions.suffix_step(
            state,
            kv_cache,
            prefix_mask,
...
def _teacher_forced_evaluation(
    ...
    *,
    ...
    eval_step = train.make_eval_step(model_def, seed_len=seed_len)
    loss_sum = 0.0
    correct = 0
    token_count = 0
...
def main(args: Args) -> None:
    ...
    dataset = data.LiberoTextualMemoryDataset(
        validation_refs,
    ...
    trained_adapter = adapter.arrays(state)
    ...
    first_observation, first_suffix_inputs, *_ = data.collate([dataset[0]])
    trained_root_cache, _ = functions.prefill_root(state, first_observation)
    ...
    trained_actions = [
        np.asarray(functions.actions(state, observation, noise))
    ...
    zero_root_cache, _ = functions.prefill_root(state, first_observation)
    zero_actions = [
        np.asarray(functions.actions(state, observation, noise))
        for observation, noise in zip(action_observations, noises, strict=True)
    ]
    ...
    root_cache, prefix_mask = functions.prefill_private(state, first_observation)
    prefix_timing = _timed(
        lambda: functions.prefill_private(state, first_observation),
    ...
    suffix_request_enabled_timing = _timed(
        lambda: functions.request_active(state, root_cache, prefix_mask, request_tokens),
        warmups=args.timing_warmups,
        repeats=args.timing_repeats,
    )
    suffix_request_disabled_timing = _timed(
        lambda: functions.request_disabled(state, root_cache, prefix_mask, request_tokens),
        warmups=args.timing_warmups,
        repeats=args.timing_repeats,
    )
