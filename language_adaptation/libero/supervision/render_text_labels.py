...
# ruff: noqa: FURB188, UP006, UP035, UP045
...
def _base_display_name(name: str) -> str:
    ...
    if base.endswith(("_cook_region", "_heating_region")):
...
def _first_stable_true(values: Sequence[bool], window: int, end: Optional[int] = None) -> Optional[int]:
    ...
    for frame in range(stop - window + 1):
...
def compile_trajectory(records: Sequence[Dict[str, Any]], stability_window: int = 3) -> List[Dict[str, Any]]:
    if not records:
        return []
    predicates = {item["id"]: item for item in records[0]["goal_predicates"] + records[0]["auxiliary_predicates"]}
    ...
    for frame, source in enumerate(records):
        ...
        row["language_components"] = {
            ...
            "textual_memory_predicate_ids": textual_memory_ids,
        }
        row["language"] = {
            "completed": _sentence(
                [_describe_state(predicates[pid], resolver, satisfied=True) for pid in completed_ids],
                "No task step has been completed yet.",
            ),
            "remaining": _sentence(
                [_describe_state(predicates[pid], resolver, satisfied=False) for pid in remaining_ids],
                "No task step remains.",
            ),
            "next": _describe_next(predicates[next_event[1]], resolver) if next_event else "Task complete.",
            "textual_memory": _sentence(
                [_describe_state(predicates[pid], resolver, satisfied=True) for pid in textual_memory_ids],
...
def compose_language(record: Mapping[str, Any], components: Sequence[str], separator: str = "\n") -> str:
    ...
    allowed = {"completed", "remaining", "next", "textual_memory"}
...
def main() -> None:
    ...
    parser.add_argument(
        "--components",
        nargs="+",
        choices=("completed", "remaining", "next", "textual_memory"),
        default=None,
    )
