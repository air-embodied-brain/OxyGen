...
# LIBERO's simulator environment uses Python 3.8.
...
def audit_file(path: Path) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if any(not str(language.get(key, "")).strip() for key in ("completed", "remaining", "next", "textual_memory")):
        ...
        textual_memory = set(components.get("textual_memory_predicate_ids", []))
        current_goal_values = {item["id"]: bool(item["satisfied"]) for item in row["goal_predicates"]}
        if not textual_memory <= goal_ids:
            issues.append(f"frame {row['frame']}: textual memory contains a non-goal predicate")
            break
        if any(not current_goal_values[predicate_id] for predicate_id in textual_memory):
            issues.append(f"frame {row['frame']}: textual memory contains a currently false predicate")
            break
    ...
    return {
        ...
        "language_values": {
            ...
            for component in ("completed", "remaining", "next", "textual_memory")
        },
    }
...
def main() -> None:
    ...
    for item in audits:
        ...
        task_catalog = catalog.setdefault(
            task_key,
            {key: set() for key in ("completed", "remaining", "next", "textual_memory")},
        )
