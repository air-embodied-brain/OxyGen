...
def _find_libero_checkout(explicit_path: Optional[Path]) -> Path:
    if explicit_path is not None:
        ...
        candidates = [oxygen_repo / "third_party" / "libero"]
...
def _goal_and_auxiliary_states(env: Any) -> Tuple[List[Sequence[str]], List[Sequence[str]]]:
    ...
    auxiliary_states = [
        [predicate, object_name] for object_name in goal_objects for predicate in ("grasped", "picked_up")
    ]
...
def _capture_pickup_references(env: Any, auxiliary_states: Sequence[Sequence[str]]) -> Dict[str, Dict[str, Any]]:
    ...
    base_env = env.env
    object_names = {
        str(state[1]) for state in auxiliary_states if len(state) > 1 and str(state[0]).lower() == "picked_up"
    }
    return {
        object_name: {
...
def _prepare_environment(
    ...
    env.reset()
    if model_xml:
        from libero.libero.utils.utils import postprocess_model_xml  # noqa: PLC0415
