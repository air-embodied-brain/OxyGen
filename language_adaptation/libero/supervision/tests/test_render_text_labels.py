# These tests also run in LIBERO's Python 3.8 environment.
# ruff: noqa: B905

from language_adaptation.libero.supervision.render_text_labels import compile_trajectory
from language_adaptation.libero.supervision.render_text_labels import compose_language
from language_adaptation.libero.supervision.render_text_labels import humanize
...
def _pickup_trajectory(goal_values, grasp_values, pickup_values):
    records = []
    for frame, (goal_value, grasp_value, pickup_value) in enumerate(zip(goal_values, grasp_values, pickup_values)):
...
def test_compiler_uses_future_stable_events():
    ...
    assert rows[7]["language"]["completed"] == ("The black bowl has been picked up; the black bowl is on the plate.")
...
def test_configurable_composition():
    ...
    assert compose_language(row, ["textual_memory"]) == "Seen."


def test_textual_memory_is_causal_stable_and_excludes_pickup():
    rows = compile_trajectory(
        _pickup_trajectory(
    ...
    assert rows[4]["language"]["textual_memory"] == "No relevant task progress is visible yet."
    assert rows[6]["language"]["textual_memory"] == "The alphabet soup is in the basket."
    assert rows[6]["language_components"]["textual_memory_predicate_ids"] == [
        "goal:00:in(alphabet_soup_1,basket_1_contain_region)"
    ]


def test_textual_memory_does_not_claim_initial_or_reverted_state():
    records = []
    ...
    rows = compile_trajectory(records, stability_window=2)
    assert rows[1]["language"]["textual_memory"] == "No relevant task progress is visible yet."
    assert rows[5]["language"]["textual_memory"] == "The microwave is closed."
    assert rows[7]["language"]["textual_memory"] == "No relevant task progress is visible yet."
...
def test_grasped_is_ignored_for_push_to_spatial_region():
    records = []
    for frame, (goal_value, grasp_value) in enumerate(
        zip(
            [False, False, False, False, False, False, True, True],
