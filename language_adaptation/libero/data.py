...
def _sample_target_frames(rows: Sequence[dict], uniform_stride: int) -> list[int]:
    selected = {0, len(rows) - 1}
    selected.update(range(0, len(rows), uniform_stride))
    previous = None
    for index, row in enumerate(rows):
        target = row["language"]["textual_memory"]
    ...
    *,
    ...
    for (suite, task), task_paths in sorted(grouped.items()):
        ...
        for path in paths:
            ...
            for frame in _sample_target_frames(rows, uniform_stride):
                row = rows[frame]
...
class LiberoTextualMemoryDataset:
    def __init__(
        self,
...
def batches(
    dataset: LiberoTextualMemoryDataset,
    *,
    batch_size: int,
    seed: int,
