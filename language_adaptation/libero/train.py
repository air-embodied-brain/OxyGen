...
def _forward_suffix_incremental(model, root_cache, prefix_mask, inputs, mask, *, seed_len: int):
    suffix_len = inputs.shape[1]
    seed_positions = jnp.arange(suffix_len)[None, :] < seed_len
    seed_logits, cache = model.append_language_suffix_block(
        root_cache,
        prefix_mask,
        jnp.where(seed_positions, inputs, 0),
        mask & seed_positions,
        adapter_active=True,
    )

    def step(current_cache, token_and_index):
        token, suffix_index = token_and_index
        logits, current_cache = model.forward_language_suffix_token(
            current_cache,
            prefix_mask,
            token[:, None],
            suffix_index,
        ...
        return current_cache, logits

    _, token_logits = jax.lax.scan(
        step,
        cache,
        (inputs[:, seed_len:].T, jnp.arange(seed_len, suffix_len, dtype=jnp.int32)),
    )
...
def _suffix_logits(model, rng, batch, *, seed_len: int, train: bool):
    observation, inputs, targets, mask, loss_mask = batch
    root_cache, prefix_mask = model.prefill_language_root(
        rng,
        observation,
        train=train,
        max_suffix_tokens=inputs.shape[1],
    )
    root_cache = jax.tree.map(jax.lax.stop_gradient, root_cache)
    logits = _forward_suffix_incremental(model, root_cache, prefix_mask, inputs, mask, seed_len=seed_len)
    return logits, targets, loss_mask & mask


def _cross_entropy(logits, targets, loss_mask):
...
def _make_train_step(model_def, tx, *, seed_len: int):
    ...
    def train_step(state, opt_state, batch, rng):
        ...
        def loss_fn(train_model):
            logits, targets, loss_mask = _suffix_logits(train_model, rng, batch, seed_len=seed_len, train=True)
            return jnp.mean(_cross_entropy(logits, targets, loss_mask))
...
def make_eval_step(model_def, *, seed_len: int):
    ...
    def eval_step(state, batch):
        model = nnx.merge(model_def, state)
        logits, targets, loss_mask = _suffix_logits(model, None, batch, seed_len=seed_len, train=False)
        losses = _cross_entropy(logits, targets, loss_mask)
        predictions = jnp.argmax(logits, axis=-1)
        correct = (predictions == targets) & loss_mask
        return jnp.mean(losses), jnp.sum(correct), jnp.sum(loss_mask)
...
def main(args: Args) -> None:
    ...
    train_dataset = data.LiberoTextualMemoryDataset(train_refs, **dataset_kwargs)
    validation_dataset = data.LiberoTextualMemoryDataset(validation_refs, **dataset_kwargs)
