...
class Policy(BasePolicy):
    ...
    def infer_text_actions_blocking_baseline(
        ...
        """Run separate action and language inference to completion.

        This is the paper-aligned isolated baseline: action and language use
        independent prefix forwards, and language generation blocks the caller
        until EOS or ``max_decoding_steps``.
        """
        if self._is_pytorch_model:
            raise NotImplementedError("The language-adapter blocking baseline currently requires JAX")
        ...
        return {
            **action_output,
            ...
            "is_finished": True,
            "active_language_requests": 0,
            "policy_timing": {
                "action_ms": (action_done - started) * 1000,
                "language_ms": (language_done - action_done) * 1000,
                "total_ms": (language_done - started) * 1000,
