...
class ArxInputs(transforms.DataTransformFn):
    ...
    Expected inputs:
    - image: dict[name, img] where img is [channel, height, width]. name must be in EXPECTED_CAMERAS.
    - state: [14]
    ...
    def __call__(self, data: dict) -> dict:
        ...
        for key in self.EXPECTED_CAMERAS:
            assert key in data['image'].keys(), f"Images must contain {key}."
            data['image'][key] = convert_image(data['image'][key])

        inputs = {
            "image": {
                "base_0_rgb": data['image']['face_view'],
                "left_wrist_0_rgb": data['image']['left_wrist_view'],
                "right_wrist_0_rgb": data['image']['right_wrist_view'],
            },
            "image_mask": {
                "base_0_rgb": np.True_,
