...
class Attention(nn.Module):
    ...
    def _update_cache(self, k, v, idx, k_cache, v_cache):
        """Update KV cache with new values - batch-aware version"""
        assert k.shape[1] == 1, "Only support kv-cache updates of length 1"

        batch_size = k.shape[0]
        cache_dtype = k.dtype

        # Vectorized update for all batch elements
        k_new, v_new = k_cache, v_cache
        for b in range(batch_size):
            indices = (b, idx[b], 0, 0)
            k_new = jax.lax.dynamic_update_slice(k_new, k[b:b+1].astype(cache_dtype), indices)
            v_new = jax.lax.dynamic_update_slice(v_new, v[b:b+1].astype(cache_dtype), indices)
