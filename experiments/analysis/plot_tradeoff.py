...
One subplot per device with available data. If a device result set is missing,
the script still writes a figure with the remaining device panels. Each subplot
has independent x and y ranges.
...
from experiments.analysis.plot_utils import PlotColors
from experiments.analysis.plot_utils import device_label_from_results_dir
from experiments.analysis.plot_utils import setup_style
...
def _try_load_metrics(results_root: Path, device_name: str) -> pd.DataFrame | None:
    path = results_root / "analysis" / "aggregate_metrics" / "metrics.csv"
    if not path.exists():
...
def _has_panel_data(df, policy, k_values, mds_values, ref_N) -> bool:
    if df is None or df.empty:
        return False
    required = {
        "setting", "policy", "num_denoise_steps", "max_decoding_steps",
        "action_frequency_hz", "language_throughput_tps", "runner",
        "arrival_pattern", "steps_per_frame",
    }
    if not required.issubset(df.columns):
        return False
    return bool(
        _ours_family(df, policy, k_values, mds_values, ref_N)
        or _baseline_curve(df, policy, "baseline", mds_values, ref_N)
        or _baseline_curve(df, policy, "parallel_mps", mds_values, ref_N)
    )
...
def _plot_tradeoff_from_specs(
    result_specs: list[dict],
    policy: str,
    k_values,
    mds_values,
    ref_N: int,
    baseline_speedup_ref: str,
    out_path: Path,
):
    panel_specs = []
    for spec in result_specs:
        df = _try_load_metrics(spec["root"], spec["label"])
        spec = {**spec, "df": df}
        if _has_panel_data(spec["df"], policy, k_values, mds_values, ref_N):
            panel_specs.append(spec)
        else:
            print(
                f"Skipping {spec['label']} panel: no rows for policy={policy}, "
                f"num_denoise_steps={ref_N}"
            )

    if not panel_specs:
        print(
            f"No tradeoff data to plot for policy={policy}, "
            f"num_denoise_steps={ref_N}"
        )
        return
    ...
    fig_width = 5.7 if n_panels == 1 else max(8.5, 3.4 * n_panels + 1.8)
    fig = plt.figure(figsize=(fig_width, 2.4))

    fig_left = 0.14 if n_panels == 1 else 0.09
    fig_right = 0.985
    fig_top = 0.88
    fig_bottom = 0.22
    panel_gap = 0.065       # gap between the two data panels
    legend_gap = 0.015      # gap between rightmost panel and legend strip
    legend_width = 0.24 if n_panels == 1 else 0.17

    width_total = fig_right - fig_left
    panel_width = (
        width_total - panel_gap * (n_panels - 1) - legend_gap - legend_width
    ) / n_panels
    height = fig_top - fig_bottom

    axes = []
    for idx in range(n_panels):
        left = fig_left + idx * (panel_width + panel_gap)
        axes.append(fig.add_axes([left, fig_bottom, panel_width, height]))

    legend_left = (
        fig_left + n_panels * panel_width
        + (n_panels - 1) * panel_gap + legend_gap
    )
    leg_ax = fig.add_axes([legend_left, fig_bottom, legend_width, height])

    for ax, spec in zip(axes, panel_specs, strict=True):
        _draw_panel(ax, spec["df"], policy, spec["label"],
                    k_values, mds_values, ref_N,
                    spec["deadline_hz"], baseline_speedup_ref)
    ...
    deadline_entry = None
    if any(spec.get("deadline_hz") for spec in result_specs):
        deadline_entry = dict(
...
def main():
    ...
    parser.add_argument("--deadline-hz", type=float, nargs="+", default=None)
    ...
    deadlines = args.deadline_hz or [None] * len(args.results_root)
    if len(deadlines) != len(args.results_root):
        raise ValueError("--deadline-hz must match --results-root length")
    result_specs = [
        {"root": root, "label": label, "deadline_hz": deadline}
        for root, label, deadline in zip(
            args.results_root, labels, deadlines, strict=True,
        )
    ]
    _plot_tradeoff_from_specs(
