"""Grid search for baseline setting."""
...
def _is_valid_combo(params: dict) -> bool:
    """Check if a parameter combination is valid. Always True for baseline."""
    return True
...
def _save_result(result: dict, results_dir: Path) -> Path:
    ...
    File path: {results_dir}/baseline/{policy_config}/denoise{N}_decode{M}.json
...
def run_grid_search(
    ...
    """Run cartesian product of search_space for the baseline setting.
