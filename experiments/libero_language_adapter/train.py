...
ADAPTER_FILTER = nnx.All(nnx.Param, nnx_utils.PathRegex(".*suffix_lora.*"))
...
def load_model(args: Args):
    ...
    state = nnx_utils.state_map(
        state, frozen_filter, lambda parameter: parameter.replace(parameter.value.astype(jnp.bfloat16))
    )
...
def _make_train_step(model_def, tx):
    @jax.jit
...
def main(args: Args) -> None:
    ...
    expected_count = 2 * 18 * 2_048 * args.rank
    if adapter_count != expected_count:
        raise ValueError(f"Expected {expected_count:,} adapter parameters, found {adapter_count:,}")
    ...
    train_step = _make_train_step(model_def, tx)
    eval_step = make_eval_step(model_def)
