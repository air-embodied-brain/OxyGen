...
2. Convert the JAX model to PyTorch format using the matching PyTorch model

Usage:
    # Just inspect keys:
    python scripts/convert_jax_model_to_pytorch.py --checkpoint-dir /path/to/checkpoint --config-name pi05_base --inspect-only

    # Convert to PyTorch:
    python scripts/convert_jax_model_to_pytorch.py --checkpoint-dir /path/to/checkpoint --config-name pi05_base --output-path /path/to/output

Example:
    # pi0_droid
    python scripts/convert_jax_model_to_pytorch.py --checkpoint-dir /home/$USER/.cache/openpi/openpi-assets/checkpoints/pi0_droid --config-name pi0_droid --output-path /home/$USER/.cache/openpi/openpi-assets/checkpoints/pi0_droid_pytorch

    # pi0_aloha_sim
    python scripts/convert_jax_model_to_pytorch.py --checkpoint-dir /home/$USER/.cache/openpi/openpi-assets/checkpoints/pi0_aloha_sim --config-name pi0_aloha_sim --output-path /home/$USER/.cache/openpi/openpi-assets/checkpoints/pi0_aloha_sim_pytorch

    # pi05_droid
    python scripts/convert_jax_model_to_pytorch.py --checkpoint-dir /home/$USER/.cache/openpi/openpi-assets/checkpoints/pi05_droid --config-name pi05_droid --output-path /home/$USER/.cache/openpi/openpi-assets/checkpoints/pi05_droid_pytorch
"""
...
import openpi.models.pi05_config
import openpi.models_pytorch.pi0_pytorch
import openpi.models_pytorch.pi05_pytorch
...
def convert_pi0_checkpoint(
    ...
    precision: str,
    output_path: str,
    model_config: openpi.models.pi0_config.Pi0Config | openpi.models.pi05_config.Pi05Config,
):
    """
    ...
    # Instantiate model
    if model_config.model_type is openpi.models.model.ModelType.PI05_O2:
        pi0_model = openpi.models_pytorch.pi05_pytorch.PI05Pytorch(model_config)
    else:
    ...
    # Copy assets folder if it exists. OpenPI checkpoints usually store
    # assets inside the checkpoint directory, but older local layouts used a
...
def main(
    ...
    *,
    ...
    if not isinstance(model_config, openpi.models.pi0_config.Pi0Config | openpi.models.pi05_config.Pi05Config):
        raise ValueError(f"Config {config_name} is not a Pi0Config or Pi05Config")
    if inspect_only:
        load_jax_model_and_print_keys(checkpoint_dir)
    else:
        if not output_path:
            print("Error: --output-path is required for conversion. Use --inspect-only to only view keys.")
            return
